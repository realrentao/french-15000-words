...
(function () {
  ...
  var DATA = {};            // gid -> 分册数据
  var AUDIO = "audio/";
  var SITE_VER = "20260916";   // 资源版本戳：更新内容后 bump 此值以破除浏览器/CDN 缓存
  ...
  var FLAT = [];            // 全局 Partie 顺序 [{g,p,gid,no,name}]
  ...
  function loadParte(gid, cb) {
    ...
    s.src = "data/sec/" + gid + ".js?v=" + SITE_VER;
    s.onload = function () {
  ...
  function render() {
    ...
    loadParte(pm.gid, function (d) {
      ...
      var h = '<div class="crumb">' + esc(gr.name) + ' › Partie ' + pm.no
        + ' <b>' + esc(pm.name) + '</b> · Section ' + sm.no + '</div>'
    ...
    loadAll(function () {
      ...
      res.slice(0, 300).forEach(function (r) {
        ...
        h += '<div class="sr-item" data-gid="' + gid + '" data-sno="' + sno + '">'
          ...
          + '<span class="sr-pos">Partie ' + r.f.no + ' · ' + esc(r.f.name) + '</span></div>';
      });
  ...
  function exportProgress() {
    ...
    try {
      ...
      a.href = url; a.download = "french-15000-progress.json";
